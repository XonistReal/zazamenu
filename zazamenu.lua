...
-- Wait for LocalPlayer - required before any UI or character code
-- NOTE: Matcha has no GetPropertyChangedSignal or CharacterAdded signals, so we poll instead.
local _Players = game:GetService("Players")
local _lp = _Players.LocalPlayer
while not _lp do task.wait(0.1); _lp = _Players.LocalPlayer end
-- Wait for the character to be in game too
...
-- -------------------------------------------------------------
--  RunService VM emulator (Matcha does not expose RunService)
-- -------------------------------------------------------------
local RunService = (function()
    ...
    local _sortedBindings = {}
    local _sortedCount = 0

    local function Signal()
        local sig = { _conns = {} }
        function sig:Connect(fn)
            local c = { fn = fn, connected = true }
            table.insert(sig._conns, c)
        ...
        function sig:Fire(...)
            ...
            while i <= #sig._conns do
                ...
                if c.connected then pcall(c.fn, ...); i = i + 1
                else table.remove(sig._conns, i) end
            end
        end
        function sig:Wait()
            local thread = coroutine.running()
            local conn
            conn = sig:Connect(function(...) conn:Disconnect(); task.spawn(thread, ...) end)
    ...
    function RS:BindToRenderStep(name, priority, fn)
        ...
        _bindings[name] = { Priority = priority or 0, Function = fn }
        _sortedCount = -1  -- invalidate cache
    end
    function RS:UnbindFromRenderStep(name) _bindings[name] = nil; _sortedCount = -1 end
    function RS:IsRunning() return _active end

    task.spawn(function()
        while _active do
            local now = os.clock()
            local dt  = math.min(now - _lastTick, 1)
            _lastTick = now

            RS.Stepped:Fire(now, dt)

            -- rebuild sorted binding cache only when count changed
            local count = 0
            for _ in pairs(_bindings) do count = count + 1 end
            if count ~= _sortedCount then
                _sortedBindings = {}
                for _, b in pairs(_bindings) do
                    if type(b.Function) == "function" then
                        table.insert(_sortedBindings, b)
                    end
                end
                table.sort(_sortedBindings, function(a, b) return a.Priority < b.Priority end)
                _sortedCount = count
            end
            for _, b in ipairs(_sortedBindings) do pcall(b.Function, dt) end
...
-- LocalPlayer and Character are guaranteed by the top-level wait above
...
-- Matcha has no CharacterAdded signal - poll for character changes instead
task.spawn(function()
    local lastChar = LocalPlayer.Character
    while true do
        task.wait(0.5)
        local current = LocalPlayer.Character
        if current ~= lastChar then
            lastChar = current
            task.wait(0.15)
            refreshChar()
        end
...
secClickerMain:addtoggle{
    ...
    Callback = function(v)
        AC.enabled = v
        if v then ac_startLoop() else ac_stopLoop() end
    end,
}
...
-- Keybind: press the bound key to snapshot current mouse position
secClickerPos:addkeybind{
    Name     = "Save Mouse Position",
    Default  = "f3",
...
--  ANTI-AFK
--
--  Every ~10 seconds (randomised ±3s) taps one of W/A/S/D for a
--  short random duration to simulate natural movement.
-- ===============================================================

local AFK = { enabled = false }

-- Windows VK codes for movement keys
local AFK_KEYS = { 0x57, 0x41, 0x53, 0x44 }  -- W A S D
...
local function afk_start()
    ...
    task.spawn(function()
        while AFK.enabled do
            -- Wait a random interval between 7 and 13 seconds
            task.wait(math.random(7, 13))
            if not AFK.enabled then break end

            -- Pick a random movement key and hold it for 0.1 - 0.4 seconds
            local key = AFK_KEYS[math.random(1, #AFK_KEYS)]
            keypress(key)
            task.wait(math.random(10, 40) * 0.01)
            keyrelease(key)
        end
    end)
end
...
secAFK:addtoggle{
    ...
    Callback = function(v)
        AFK.enabled = v
        if v then afk_start() else afk_stop() end
    end,
}


-- ===============================================================
--  FLY  (velocity-based, Matcha-compatible)
--
--  Sets HRP.AssemblyLinearVelocity directly every Heartbeat.
--  We own the full velocity vector each frame, so gravity can't
--  accumulate - if no vertical key is held, Y velocity is forced
--  to 0 (hover). This gives the tween/smooth feel without needing
--  to write to Position (which Matcha may not support as writable).
--
--  Forward direction: Camera sits behind HRP in third-person.
--  (HRP.Position - Camera.Position) projected to XZ = forward.
...
local function fly_stop()
    ...
    if HRP then
        ...
        HRP.CanCollide = Fly._savedCanCol
    end
end
...
local function fly_start()
    ...
    Fly.enabled      = true
    Fly._savedCanCol = HRP.CanCollide
    ...
    Fly._conn = RunService.Heartbeat:Connect(function(dt)
        ...
        -- Camera sits behind/above HRP in third-person Roblox.
        -- HRP - Camera projected to XZ gives us the look/forward direction.
        local camPos = Camera.Position
        local hrpPos = HRP.Position

        local fdx = hrpPos.X - camPos.X
        local fdz = hrpPos.Z - camPos.Z
        local fdLen = math.sqrt(fdx*fdx + fdz*fdz)

        local fx, fz  -- forward unit
        if fdLen > 0.01 then
            fx = fdx / fdLen
            fz = fdz / fdLen
        else
            fx, fz = 0, -1  -- fallback: camera directly above
        end
        -- right = rotate forward 90deg on Y: (fz, 0, -fx)
        local rx, rz = fz, -fx

        -- Desired velocity components from held keys
        ...
        -- Normalize XZ if moving diagonally
        local hLen = math.sqrt(tx*tx + tz*tz)
        if hLen > 0.001 then
            tx = (tx / hLen) * Fly.speed
            tz = (tz / hLen) * Fly.speed
        end

        -- Smooth lerp toward target (snappy but not instant)
        local lf = 1 - (0.5 ^ (dt * 14))
        Fly._vx = Fly._vx + (tx - Fly._vx) * lf
        Fly._vy = Fly._vy + (ty - Fly._vy) * lf
        Fly._vz = Fly._vz + (tz - Fly._vz) * lf

        -- Apply full velocity vector - this overrides gravity each frame
...
secFlyMain:addtoggle{
    ...
    Default  = false,
    Callback = function(v)
        if v then fly_start() else fly_stop() end
    end,
}
...
-- ===============================================================
--  ESP  (dual-mode: named instances + folder scan)
--
--  Two sources feed into one shared drawing pool each frame:
--
--  1. NAMED TARGETS  - user types any instance name that exists
--     anywhere in Workspace (searched recursively via
--     FindFirstChild). Works for single models, parts, unions.
--     Each name added gets its own row with a remove button.
--
--  2. FOLDER SCAN    - user types a folder name in Workspace.
--     Every direct child that is a BasePart, Model, or
--     UnionOperation gets picked up automatically - no need
--     to add each one manually.
--
--  Both sources are merged, distance-filtered, sorted closest-
--  first, capped by Max Shown, then rendered from a shared pool
--  of Drawing objects (no per-target allocation each frame).
-- ===============================================================
...
-- Named targets: key = instance name string, value = true
local espNamedTargets = {}

-- Folder scan: set of folder names to scan each frame
local espFolderTargets = {}

-- Drawing pool: created on demand, reused each frame
local espPool     = {}
local espPoolSize = 0
...
local function newESPDrawings()
    ...
    return { box=box, nameText=nameText, distText=distText,
             hpBarBG=hpBarBG, hpBarFG=hpBarFG }
end
...
local function getDrawingSet(index)
    while espPoolSize < index do
        espPoolSize = espPoolSize + 1
        espPool[espPoolSize] = newESPDrawings()
    end
...
local function startESPLoop()
    ...
    espRenderConn = RunService.Heartbeat:Connect(function()
        ...
        for i = 1, shown do
            ...
            d.distText.Position = Vector2.new(bx + bw*.5, by + bh + 3)
        ...
        for i = shown+1, espPoolSize do
            hideESPDrawings(espPool[i])
        end
    end)
end
...
local function stopESPLoop()
    ...
    for i = 1, espPoolSize do hideESPDrawings(espPool[i]) end
end
...
-- Distance / count sliders
secESPConf:addslider{
    Name     = "Max Shown",
    Minimum  = 1,
    Max      = 50,
    Step     = 1,
    Default  = 20,
    Suffix   = " targets",
    Callback = function(v) ESP_MAX_SHOWN = v end,
}
...
secESPConf:addslider{
    ...
    Default  = 500,
    Suffix   = " studs",
    Callback = function(v) ESP_MAX_DIST = v end,
}

-- Named targets: textbox + Add + list of remove buttons
local tbNamedInst = secESPNamed:addtextbox{
    Name        = "Instance Name",
    Placeholder = "e.g.  Zombie  or  TreasureChest",
    Callback    = function() end,
}

-- Rebuild the named-targets list
local secESPNamedList  -- forward declare for use inside rebuildNamedList
local function rebuildNamedList()
    secESPNamedList:clear()
    local any = false
    for name, _ in pairs(espNamedTargets) do
        any = true
        secESPNamedList:addbutton{
            Name     = "remove:  " .. name,
            Callback = function()
                espNamedTargets[name] = nil
                rebuildNamedList()
            end,
        }
    end
    if not any then
        secESPNamedList:addbutton{
            Name = "(none - add one above)",
            Callback = function() end,
        }
    end
end

secESPNamed:addbutton{
    Name     = "Add Named Target",
    Callback = function()
        local name = (tbNamedInst.text or ""):match("^%s*(.-)%s*$")
        if name == "" then return end
        if espNamedTargets[name] then
            lib:notify("Already tracking: " .. name)
            return
        end
        espNamedTargets[name] = true
        rebuildNamedList()
        lib:notify("ESP added: " .. name)
    end,
}

secESPNamed:addbutton{
    Name     = "Clear All Named",
    Callback = function()
        espNamedTargets = {}
        rebuildNamedList()
    end,
...
-- Create the list section AFTER the buttons so it appears below them
secESPNamedList = tabESP:section("named list", false)
rebuildNamedList()

local tbFolderName = secESPFolder:addtextbox{
    Name        = "Folder Name",
    Placeholder = "e.g.  Enemies,  Collectibles",
    Callback    = function() end,
}

local secESPFolderList
local function rebuildFolderList()
    secESPFolderList:clear()
    local any = false
    for name, _ in pairs(espFolderTargets) do
        any = true
        secESPFolderList:addbutton{
            Name     = "remove:  " .. name,
            Callback = function()
                espFolderTargets[name] = nil
                rebuildFolderList()
            end,
        }
    end
...
secESPFolder:addbutton{
    Name     = "Add Folder",
    Callback = function()
        local name = (tbFolderName.text or ""):match("^%s*(.-)%s*$")
        if name == "" then return end
        if espFolderTargets[name] then
            lib:notify("Already scanning: " .. name)
            return
        end
        espFolderTargets[name] = true
        rebuildFolderList()
        lib:notify("Folder added: " .. name)
    end,
}

secESPFolder:addbutton{
    Name     = "Clear All Folders",
    Callback = function()
        espFolderTargets = {}
        rebuildFolderList()
    end,
